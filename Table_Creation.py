...
def Table_Creation(years, months, hours, elevation_def):
...
#"""
# Average Tables and Value Tables for every era_length year period between 1950-2022 for every airport
...
def Table_Creation_Era(years, months, hours, elevation_def, era_length):
    airport_count = 0
    slopes = []
    r_val = []
    p_val = []
    ...
    for era_beginning in range(1950, last_starting_year):
        era_count += 1
        years = range(era_beginning, era_beginning+era_length)
        ...
        loading = "["+ len(range(1950, era_beginning))*"1"+len(range(era_beginning, last_starting_year-1))*"0"+ "]"
