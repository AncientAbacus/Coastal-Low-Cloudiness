...
for era_beginning in range(1950, 1994):
    ...
    for file in os.listdir('Airport_Data_Tables'):
        ...
        if airport_name in airport_acronyms:
            ...
            airport_count += 1
            airports.append(airport_name)
            current_airport_data = calculate_airport_data('Airport_Data_Tables/'+file, years, months, hours, elevation, 1000)
            slopes.append(current_airport_data[1])
            r_val.append(current_airport_data[2])
            p_val.append(current_airport_data[3])
            PDO_r_val.append(current_airport_data[4])
...
for era_beginning in range(1950, 1994):
    ...
    avg_table_title = "Airport_CLC_Summary_Table_Years_" + str(years[0]) + "_to_" + str(years[-1]) + "_Months_" + listToString(months) + "_Hours_" + listToString(hours) + "_Elevation_Definiton_" + str(elevation_def)

    value_table_title = "Airport_Values_Summary_Table_Years_" + str(years[0]) + "_to_" + str(years[-1]) + "_Months_" + listToString(months) + "_Hours_" + listToString(hours) + "_Elevation_Definiton_" + str(elevation_def)
